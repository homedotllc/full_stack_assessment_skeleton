...
1. **User Table**: Stores unique user data (`id`, `username`, `email`), ensuring no duplicate usernames.
2. **Home Table**: Stores unique home data (`id`, `address`, `state`, etc.), preventing address duplication.
3. **User-Home Relationship Table**: Tracks user-home interactions using foreign keys, linking `user_id` and `home_id`. A composite primary key ensures the uniqueness of each relationship.

### Data Migration:

- **Users** and **homes** are extracted from the original dataset using `SELECT DISTINCT`, ensuring no duplicates.
- The relationships between users and homes are restored via joins on the `user_home` table.

Complete summary is present [here](sql/README.md)
...
### React SPA Solution Overview

The solution focuses on building a **React SPA** with effective state management and data-fetching mechanisms using **Redux Toolkit** and **RTK Query** to ensure smooth functionality, responsiveness, and maintainability.

### Key Steps:

1. **Homes for User Page**:

   - A dropdown lets users select a specific user, displaying related homes in responsive cards.
   - Each card includes an "Edit User" button to modify the users associated with that home.

2. **Edit User Modal**:
   - This modal shows all users associated with a home using checkboxes. It allows modifying these associations and includes validation to ensure at least one user is selected before saving.
3. **State Management**:
   - Redux Toolkit is used for managing global states such as selected users and fetched homes, simplifying state handling and reducing boilerplate code.
4. **Data Fetching**:
   - RTK Query is employed for API requests, handling loading states, caching, and errors efficiently. This reduces the need for manual API logic and optimizes performance.

### User Interaction Flow:

- **User Selection**: The user selects a user from the dropdown to view their associated homes.
- **Editing User Associations**: The "Edit User" modal allows modifying user-home associations, with changes reflected in real-time after saving.

### Benefits:

- **State Management Efficiency**: Redux Toolkit makes state handling scalable and easier to maintain, especially as the app grows.
- **Optimized Data Fetching**: RTK Query improves performance by caching API responses, handling errors gracefully, and ensuring data consistency.
- **Responsive Design**: The app is built with responsive layouts, ensuring compatibility across devices.
- **Maintainability and Scalability**: The component-based structure and efficient state/data management enable easy updates and future feature additions.

Complete summary is present [here](frontend/README.md)
...
This backend API, built using **Node.js** with **TypeORM**, focuses on managing users and homes with an emphasis on efficient **database design** and interaction. The solution highlights entity relationships, idempotent updates, and scalable querying.
...
1. **Home Entity (`home.entity.ts`)**:
   ...
   - Represents the `home` table.
   - **Columns**: `id`, `street_address`, `state`, `zip`, `sqft`, `beds`, `baths`, `list_price`.
   - **Relations**:
     - One-to-many with `UserHome` join table to handle many-to-many relationships with users.
...
2. **User Entity (`user.entity.ts`)**:
   ...
   - **Relations**:
     - One-to-many with `UserHome` to map user-home associations.

3. **UserHome Entity (`userHome.entity.ts`)**:
   - Join table `home_user` for many-to-many relationships.
   - **Columns**: `userId` and `homeId` (Primary Keys, Foreign Keys).
   - **Relations**:
     - **ManyToOne** with both `User` and `Home`, enabling the association between homes and users.
...
1. **`/user/find-all`**:
   - Queries the `user` table to retrieve all users.
2. **`/home/find-by-user`**:

   - Uses the `UserHome` join table to fetch homes linked to a specific user.
   - **Pagination**: Limits results to 50 homes per page.

3. **`/user/find-by-home`**:

   - Retrieves users linked to a home via the `UserHome` entity.

4. **`/home/update-users`**:
   - Updates user-home associations by clearing old records in `UserHome` and inserting new ones, ensuring **idempotency**.

---

### Key Database Solutions

1. **Join Table (UserHome)**:

   - The `UserHome` entity effectively models the many-to-many relationship between users and homes, ensuring flexibility in how data is stored and retrieved. This avoids data duplication and maintains normalization.

2. **Idempotent Updates**:

   - The `/home/update-users` endpoint ensures consistency by **deleting all existing associations** and **inserting new ones** in a single transaction. This guarantees that each update produces the same result, maintaining **data integrity**.

3. **Optimized Queries**:

   - **TypeORM QueryBuilder** is leveraged to run efficient SQL queries. By using optimized batch operations, the API reduces database load, ensuring scalable performance even with large datasets.

4. **Transactions**:
   - Critical operations, like updating user-home relationships, are wrapped in **atomic transactions**. This ensures that database state remains valid even if errors occur during the update process, preventing partial updates.

Complete summary is present [here](backend/README.md)
