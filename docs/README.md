...
- Key Problem's Solution
  ...
  1. Created user and home table with uniqe key username and street_address respectivly to normalize the data structure. This solves the problem of data reduncency, delete, update and insertion anomaly.
  2. For many to many relationship between user and home, join table with just user_id and home_id column is created by altering pre existing table user_home. With this approach we are not creating one extra table.
  3. Implemented Step-by-step process to add new column, update data and remove unnecessary columns from the user_home table.
  4. All these step are performed using SQL queries that is placed in sql script file named `99_final_db_dump.sql` inside `sql` directory. And we do not have to run this script manually.

- Instruction

  1. To run `99_final_db_dump.sql` script, copy it to docker image in folder `/docker-entrypoint-initdb.d/`. Any script inside this directory of docker image will run automatically when container first starts.
  2. To Review the output that tables are created and populated correctly. We will connect to mysql container using following commands.

  ```javascript
  //Connect to container shell
    docker exec -it mysql_ctn /bin/sh
  // Inside docker shell run command to connect to mysql server with password prompt
    mysql -u db_user -p
  // Use the home_db;
    use home_db;
  // To see if all tables are created or not
    show tables;
  // To see table structure and run other query
    DESCRIBE user;
    DESCRIBE home;
    DESCRIBE user_home;
    SELECT COUNT(*) FROM user;
    SELECT COUNT(*) FROM home;
    SELECT COUNT(*) FROM user_home;
  ```
